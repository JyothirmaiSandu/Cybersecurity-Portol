...
# Track the current load balancer index for round-robin
current_lb_index = 0

# Round-robin load balancing function
def get_next_load_balancer():
    global current_lb_index
    if load_balancers:
        lb = load_balancers[current_lb_index]
        current_lb_index = (current_lb_index + 1) % len(load_balancers)  # Move to the next load balancer
        return lb
    return None

@app.route('/distribute_request')
def distribute_request():
    lb = get_next_load_balancer()
    if lb:
        return jsonify({
            "message": f"Request sent to Load Balancer: {lb['name']} with IP: {lb['ip']}"
        })
    else:
        return jsonify({"error": "No active load balancers available"}), 500
@app.route('/')
def index():
